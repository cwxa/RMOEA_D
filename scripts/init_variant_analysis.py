...
3. **结论只用相对差**（dHV / 胜负 / p / dz / rel%）：这些量跨**实例**可比
   （各实例各有自己的盒），但跨**臂集**不可直接比 —— 相对差同样随盒漂移
   （缺陷 25：Mk10 上 `I_mwr` 的 +8.64% 在另一套臂集下只有 +0.690%）。
   凡引用必须声明臂集，详见 `scripts/hv_box.py` 的模块说明。
...
def main():
    ...
    W("★ 全部结论只用相对差；跨实例可比，但**跨臂集不可比**（相对差随盒漂移）。")
    W()
