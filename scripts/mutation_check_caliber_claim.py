...
M5 另有用途（缺陷 53）：它把同一族断言写进 `readme.md`，用来证明**扫描覆盖面**
...
MUTATIONS = [
    ...
    # 覆盖面：readme.md（仓库门面）直到 2026-09-21 都不在**任何**口径守卫的扫描范围内
    # （缺陷 53：锁用 os.listdir 不递归、也不含根级文档）。这条变异证明它现在被看到了。
    ("M5 readme.md（根级门面）退回'只有 ΔHV/p/wins 可跨批次比'",
     "readme.md",
