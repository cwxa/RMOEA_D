...
结论用法
--------
* 报「某组件有没有用」：两个口径给同样的 **符号 / wins / p**，都可；
    ...
    for inst in per:
        ...
        for a, b in pairs:
            if a not in hv_inst or b not in hv_inst:
                continue
            rec = {}
            for tag, hv in (("inst", hv_inst), ("box", hv_box)):
            ...
            if np.isfinite(ratio):
                print("   %-16s → 盒口径把 rel%% 放大了 %.1f 倍" % ("", ratio))

    print("\n" + "=" * 100)
    print("跨实例汇总（rel% 直接平均；各实例效应同号时才有意义）")
    print("=" * 100)
    print("%-24s %-14s %-14s %-8s %-10s" %
          ("臂对", "实例边界 rel%", "盒口径 rel%", "放大", "wins(pooled)"))
