...
**⚠ 连 ΔHV / p / wins 也随盒变**（缺陷 25/30）——换臂集会改变 run 之间的
相对大小、p 值，甚至**最优臂的归属**（缺陷 39：同一份数据里 T 的最优档被
盒口径从 T15 颠倒成 T50，Friedman p 从 0.54 变成 4.7e-05）。因此本模块的盒
只用于**同一臂集内**的分析；凡引用任何数字（含 p、wins）都必须同时声明
**臂集**与**口径**，跨批次引用一律无效。
"""
import hashlib
import io
