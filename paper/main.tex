...
MOEA/D\cite{zhang2007moead} 是这一类的代表。
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}p{0.20\linewidth}p{0.72\linewidth}@{}}
    ...
    MIX3 / MWR & 两种初始化（起点构造），后者是本文要检验的候选 \\
    ...
    置换零假设 & 打乱实例与目标量的对应、重复多次，用于家族校正与事后检验 \\
    \bottomrule
  \end{tabular}
\end{table}
...
规模相差悬殊的实例：在开发集内，仅\textbf{负载下界}一项，最大与最小就相差
$\BndWlSpan\times$（Mk08 的 $\BndMaxLoWl$ 对 Mk02 的 $\BndMinLoWl$），
最大完工时间的下界也相差 $\BndMsSpan\times$。于是所有实例的读数都被"拉"进
相近区间，读出"所有变体都近乎完美"的假象。凡按 $(\text{臂},\text{seed})$
在多个实例上摊平再取极值的写法都会踩这一点。
...
\begin{enumerate}\itemsep2pt
  \item \textbf{放大倍数不是常数}：按臂对汇总（比值之比，\S\ref{sec:stat}）从
        ...
        格子里，$|\Delta \hv|\le \CalibSmallThr\%$（"接近零"的显式阈值）的 $\CalibSmallN$ 个
...
都要跑多个种子。本文取种群规模 $N_p=\SetNp$、代数 $G=\SetG$、每个配置 \SetSeeds 个随机
种子（42--71），$\mathrm{ref}=(\SetRefLo,\SetRefLo)$。所有组间比较都做\textbf{同 seed 配对}，
以消掉"模糊时间随 seed 变"带来的实例内噪声。响应面实验（\S\ref{sec:surface}）
另在 $G=\SetGMax$ 上限下按墙钟预算分档，用于观察"算力变大时效应如何变"。

\subsection{统计口径}\label{sec:stat}
同一组比较给两套读数：\textbf{实例级}（先在实例内对 30 个 seed 取均值，$n=10$ 或
$n=66$）与 \textbf{run 级}（逐 seed 配对，$n=\RunN$）。检验用 Wilcoxon 符号秩，
...
MWR 是本文提出的一个初始化变体：决定工序先后时按"剩余工时最多者优先"的规则排列
（一种常见的派工启发式），只在"选哪台机器"这一维度上保留原算法的随机性。要问
"该不该用它"，必须把对照设成\textbf{论文自己的 MIX3 初始化}（目标 B）；把它对照
随机初始化（目标 A）回答的则是另一个问题。表~\ref{tab:targets} 与
图~\ref{fig:targets} 同时给出两者。
...
(2) 对 $T$ 的动作空间做穷举式扫描（把所有候选的 $T$ 值逐个试完，
含空间外的更大候选值）：空间内最优与最差的差只有 $\TscanGapPct\%$
