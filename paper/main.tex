...
\begin{itemize}\itemsep2pt
  ...
  \item \textbf{Wilcoxon 符号秩检验}：配对差往往不服从正态分布，用不了 $t$ 检验，
        ...
        "某组件有增益"严格定义为"\textbf{配对检验 $p<0.05$ 且方向为正}"，
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}p{0.20\linewidth}p{0.72\linewidth}@{}}
    ...
    ref & HV 的参考点，本文取 $(1.02,1.02)$（归一化后） \\
...
都要跑多个种子。本文取种群规模 $N_p=100$、代数 $G=200$、每个配置 30 个随机
种子（42--71），$\mathrm{ref}=(1.02,1.02)$。所有组间比较都做\textbf{同 seed 配对}，
