...
\begin{itemize}\itemsep2pt
  ...
  \item 提出零代探针门控：$\lam$（初始前沿 HV 杠杆）与净增益在开发集上完全分离；
        在 66 个 Hurink 实例的预注册留出集上给出无偏的命中/误放/漏放计数
        （\S\ref{sec:hurink}），并回答"门控需要多少 seed"（\S\ref{sec:seedvar}）。
...
\subsection{局限}
\begin{itemize}\itemsep2pt
  \item 留出集只有 Hurink $e$-data 一个族；不同族（如 $r$/$v$-data）的弹性比更低，
        探针的判别力是否保持未测。
  \item 门限 $\lam\ge\HKthr\%$ 是在 $n=10$ 的开发集上挑出的，虽然留出集验证无偏，
        但门限值本身没有跨族校准；\S\ref{sec:seedvar} 已给出边界实例的不稳定性，
        实际使用时应把"距门限的距离"一并报告。
