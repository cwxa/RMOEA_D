\begin{table}[htbp]
  ...
  \begin{tabular}{rrrrrrrrrr}
    \toprule
    \multicolumn{2}{c}{} & \multicolumn{4}{c}{$\lambda_0$ 的子集标准差} & \multicolumn{4}{c}{与 \SetSeeds-seed 判定的一致率} \\
    \cmidrule(lr){3-6}\cmidrule(lr){7-10}
    实例 & $\lambda_0(\SetSeeds)$ & $n{=}1$ & $n{=}3$ & $n{=}5$ & $n{=}10$ & $n{=}1$ & $n{=}3$ & $n{=}5$ & $n{=}10$ \\
    \midrule
    ...
    \bottomrule
  \end{tabular}
  \par\vspace{3pt}\footnotesize\begin{minipage}{\linewidth}对每个实例枚举/抽样 $n$ 个 seed 的子集，用与正式探针完全相同的估计量（先对 seed 求均值再作比）重算 $\lambda_0$，再看判定是否与 \SetSeeds-seed 一致。门限 $\HKthr\%$；子集数见 \texttt{logs/probe\_seed\_var.json}。\end{minipage}
\end{table}
